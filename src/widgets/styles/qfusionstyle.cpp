    ...
    case PE_IndicatorCheckBox:
        ...
        if (const QStyleOptionButton *checkbox = qstyleoption_cast<const QStyleOptionButton*>(option)) {
            ...
            if (checkbox->state & State_NoChange) {
                ...
                penWidth = qMax(penWidth , qreal(0.18 * rect.height()));
                penWidth = qMin(penWidth , qreal(0.30 * rect.height()));
    ...
    case CE_MenuItem:
        ...
        if (const QStyleOptionMenuItem *menuItem = qstyleoption_cast<const QStyleOptionMenuItem *>(option)) {
            ...
            int checkcol = qMax(menuItem->rect.height() * 0.7,
                                      qMax(qreal(menuItem->maxIconWidth * 1.0), dpiScaled(17))); // icon checkbox's highlihgt column width
            if (
#if QT_CONFIG(combobox)
